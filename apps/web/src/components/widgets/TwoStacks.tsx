...
export function TwoStacks() {
  ...
  const TOP_MAX = 1000;
  const xLin = (v: number) =>
    PAD_L + ((v - TOP_MIN) / (TOP_MAX - TOP_MIN)) * innerW;

  // Log (bottom) axis: 1..1000.
  const LOG_LO = 0; // log10(1)
  const LOG_HI = 3; // log10(1000)
  const xLog = (v: number) =>
    PAD_L + ((Math.log10(v) - LOG_LO) / (LOG_HI - LOG_LO)) * innerW;
    ...
    <div className="widget">
      ...
      <div className="widget-readout">
        ...
        <div className="readout-row">
          ...
          <span className="readout-value">
            {(Math.log10(a) + Math.log10(b)).toFixed(2)}
          </span>
        </div>
      ...
      <div className="widget-controls">
        <label className="ctrl">
          <span>
            <span className="swatch" style={{ background: "var(--acc)" }} aria-hidden />
            a
          </span>
          <input
            type="range"
        ...
        <label className="ctrl">
          <span>
            <span className="swatch" style={{ background: "var(--acc-deep)" }} aria-hidden />
            b
          </span>
          <input
            type="range"
      ...
      <div className="widget-caption">
        {pick(
          language,
          <>
            Drag <b>a = 2</b> and <b>b = 3</b>. The marker lands on <b>6</b> —
            but you never multiplied. You added two log-distances. Drag <b>b</b>{" "}
            to <b>5</b>. The marker jumps to <b>10</b>. Same trick.
          </>,
          <>
            <b>a = 2</b>, <b>b = 3</b>으로 드래그. 마커가 <b>6</b>에 떨어진다 —
            곱한 적은 한 번도 없다. 두 로그 거리를 더했을 뿐. <b>b</b>를{" "}
            <b>5</b>로 드래그하면 마커가 <b>10</b>으로 점프. 같은 트릭.
          </>
        )}
      </div>
    </div>
