...
export function PizzaSlider() {
  ...
  const yScaleLin = (b: number) => H - PAD_B - ((b - yMinLin) / (yMax - yMinLin)) * (H - PAD_T - PAD_B);
  ...
  const path = points.map((p, i) => `${i === 0 ? "M" : "L"} ${xScale(p.year).toFixed(2)} ${yScale(p.bal).toFixed(2)}`).join(" ");
    ...
    <div className="widget">
      ...
      <div className="widget-readout">
        <div className="readout-row">
          <span className="readout-label">{pick(language, "Laszlo's $41 (P)", "라즐로의 $41 (P)")}</span>
        ...
        <div className="readout-row readout-result">
          <span className="readout-label">F = P(1+r)<sup>t</sup></span>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="widget-plot" role="img">
        <rect x={PAD_L} y={PAD_T} width={W - PAD_L - PAD_R} height={H - PAD_T - PAD_B} className="plot-bg" />
        {yTicks.map((v, i) => (
          <g key={i}>
            <line x1={PAD_L} y1={yScale(v)} x2={W - PAD_R} y2={yScale(v)} className="plot-grid" />
            <text x={PAD_L - 8} y={yScale(v) + 3} className="plot-tick">{formatCurrency(v)}</text>
          </g>
        ))}
        {yearMarks.map((y, i) => (
          <g key={i}>
            <line x1={xScale(y)} y1={H - PAD_B} x2={xScale(y)} y2={H - PAD_B + 4} className="plot-axis" />
      ...
      <div className="widget-controls">
        <label className="ctrl">
          <span>{pick(language, "principal P (BTC value, May 2010)", "원금 P (BTC, 2010년 5월)")}</span>
          <input type="range" min={1} max={1000} step={1} value={P} onChange={(e) => setP(+e.target.value)} />
          <span className="ctrl-val">{formatCurrency(P)}</span>
        </label>
        <label className="ctrl">
          <span>{pick(language, "annual rate r (BTC actual ≈ 189%)", "연이율 r (BTC 실제 ≈ 189%)")}</span>
          <input type="range" min={0} max={2.5} step={0.01} value={r} onChange={(e) => setR(+e.target.value)} />
          <span className="ctrl-val">{(r * 100).toFixed(0)}%</span>
        </label>
        <label className="ctrl">
          <span>{pick(language, "years t (since 2010)", "햇수 t (2010년부터)")}</span>
          <input type="range" min={0} max={40} step={1} value={t} onChange={(e) => setT(+e.target.value)} />
      ...
      {frozen && (
        <div className="freeze-panel">
          <div className="freeze-title">
            {pick(language, "Frozen — solve on paper, then check.", "고정됨 — 종이로 풀어보고 답을 확인하라.")}
          </div>
          <div className="freeze-grid">
            <div><b>P</b> = {formatCurrency(frozen.P)}</div>
            <div><b>r</b> = {(frozen.r * 100).toFixed(1)}%</div>
            <div><b>t</b> = {frozen.t.toFixed(0)} yr</div>
            <div className="hidden-target"><b>F = ?</b></div>
          </div>
          <details>
      ...
      <div className="widget-caption">
        {pick(
          language,
          <>Try this: drop r to <b>10%</b> (SPY's long-run rate). The line on the log axis goes shallow — barely tilted. Crank r back to <b>189%</b>. It steepens. <em>The slope on a log axis is the rate.</em> You're reading exponential growth as one number: how steep.</>,
          <>해보기: r을 <b>10%</b>(SPY 장기 수익률)까지 내려보자. 로그 축 위의 선이 거의 평평해진다. 다시 <b>189%</b>로 올려보자. 선이 가팔라진다. <em>로그 축의 기울기 = 이율.</em> 지수 성장을 단 하나의 수 — '얼마나 가파른가' — 로 읽고 있는 셈이다.</>
        )}
      </div>
    </div>
  );
